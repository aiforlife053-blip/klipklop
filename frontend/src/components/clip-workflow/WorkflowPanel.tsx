  ...
  return <article className="overflow-hidden rounded-xl border border-line bg-secondary/40">
    ...
    <div className="space-y-3 p-4">
      ...
      {ready ? <>
        ...
        <label className="block text-xs font-bold">Hook text — maksimal 8 kata<input value={text.hook_text} onChange={(e) => props.onUploadTextChange(clip.clip_id, { ...text, hook_text: e.target.value })} className={field} aria-invalid={words > 8} /></label>
        {words > 8 && <p role="alert" className="text-xs text-destructive">Hook maksimal 8 kata.</p>}
